...
try:
    import os, texteditor, dec, find, gcwd_, runpy, colorama, pyplus, math, subprocess, music, shutil;
...
def cmd(command):
    ...
    if(command[0:5] == "rmdir"):
        try:
            firstSpace = command.find(" ");
            if(firstSpace == -1): console.error("Error: You must enter a path."); return 1;
            directoryName = command[firstSpace:].lstrip().rstrip();
            if(directoryName == ""): console.error("Error: Path does not exist ('')"); return 1;
            allSlashes = True;
            for ind,chr in enumerate(directoryName):
                if(chr != "\\" and chr != "/"):
                    allSlashes = False;
                    break;
            if(allSlashes): console.error("Error: You must enter a path."); return 1;
            fullDir = Vars.directory+"/"+directoryName;
